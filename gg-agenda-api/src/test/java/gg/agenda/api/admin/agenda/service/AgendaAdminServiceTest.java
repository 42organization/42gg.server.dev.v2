	...
	class UpdateAgenda {
		...
		void updateAgendaSuccessAdmin() {
			...
			Agenda agenda = Agenda.builder()
				.title("title").content("content").posterUri("posterUri").hostIntraId("hostIntraId")
				.deadline(LocalDateTime.now().minusDays(3)).startTime(LocalDateTime.now().plusDays(1))
				.endTime(LocalDateTime.now().plusDays(3)).isOfficial(true).isRanking(true)
				.minTeam(5).maxTeam(10).currentTeam(teams.size()).minPeople(1).maxPeople(10)
				.location(Location.SEOUL).status(AgendaStatus.ON_GOING).build();
			...
			AgendaAdminUpdateReqDto agendaDto = AgendaAdminUpdateReqDto.builder()
				...
				.isOfficial(false).isRanking(false).agendaStatus(AgendaStatus.CONFIRM)
				.agendaDeadLine(LocalDateTime.now()).agendaStartTime(LocalDateTime.now().plusDays(3))
				.agendaEndTime(LocalDateTime.now().plusDays(5)).agendaLocation(Location.MIX)
				.agendaMinTeam(2).agendaMaxTeam(20).agendaMinPeople(2).agendaMaxPeople(20).build();
		...
		void updateAgendaFailAdminWithCannotChangeTeamPeopleLimit() {
			// given
			// when
			// then
		}
